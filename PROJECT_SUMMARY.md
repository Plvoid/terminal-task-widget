...
**Wave 2 — themes: SPEC WRITTEN, not implemented.** See **`PLAN_wave2_themes.md`** — that file is binding and supersedes this paragraph. Decided: **preset palette** (each preset ships its own accent set), **per-preset deadline ramp** with an off switch, driven by `/theme` commands because invariant #1 forbids adding focusable elements. The load-bearing finding is that `green-400` (≈142) and `red-400` (≈0) *are* the two endpoints `useDeadlineColor` ramps between, so the accents already collide with the theme at both ends of the ramp — which is what rules out a free hue picker and forces the four accent roles to become CSS variables before any preset can ship. A **full** custom preset (user supplies all five colors) is acceptable and is a later fold-in; a single hue knob is not.
