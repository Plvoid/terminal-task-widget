...
1. **`sel` is still positional and unreconciled** — an undo, or the backlog `[ x ]` pill (whose keyboard twin *does* fix up selection), can leave the highlight on a path that now belongs to a different node. Nothing crashes (`rowIndexOf` → −1, `applyMove` bounds-checks, `nodeAt` null-checks) but the highlighted row and the row the next keystroke acts on can differ. Same class as Fix A; give `sel` the id treatment.
...
**Wave 3 — nesting: DONE.** Remaining fold-ins that were queued with it and are still open: `/clear` scoped to completed-only, ↑/↓ through the command-hint dropdown, and residual issue #1 above.
...
- **History was rebuilt 2026-08-04** to drop local absolute paths and the username from the public repo. The pre-rebuild history survives only in the local bundle.
