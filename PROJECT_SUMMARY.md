...
**Persistence (3 layers)**: localStorage (`geek-tasks`, `geek_backlog`, `geek-archive`, `geek-deadline`, `geek-hotkey`, `geek-daily`, `geek-last-date`, `geek-autostart-init`, `geek-autostart-announce`, `geek-flatten-announce`, `geek-surface-skip`, `geek-theme`, `geek-ramp`) → debounced 800ms disk mirror `Documents/TerminalTasks/state.json` (full restore on boot if localStorage is empty = disaster recovery) → `/export` timestamped JSON snapshots.
...
5. **Childless parents keep a derived `[x]`** — `recomputeCompletion` skips nodes with no children, so deleting a completed parent's last child leaves it ticked forever and counted as a completed leaf. **Decided 2026-08-06: this is intended, not a bug.** "A parent is complete once all its subtasks are" is the original design; the parent ticked because the work was genuinely done, and deleting the breakdown afterwards does not undo it. It therefore stays a completed leaf, counts toward the progress bar, and is archived. Do not "fix" this. Noted here because `completedLeaves` (the rollover archiver) now picks these up at any depth, where the old top-level-only filter would have missed a nested one.
6. `normalizeTree` silently drops non-object array entries (bare strings) without counting them in `flattened`.
7. **Promote clears the selection** — `Space` on a backlog row promotes and then calls `setSel(null)`, so a second `Space` does nothing and `↓` jumps to `rows[0]` (the first *task*), where a third `Space` **completes it**. Promoting two items in a row therefore walks the user into completing something. The fix has two precedents in the file — `deleteNode` and backlog-`Backspace` both land on the vacated slot. Left open deliberately, not yet decided.
8. **Nested subtask selection reads weakly** — a selected depth ≥1 row gets only `bg-gray-700/50`, no border, no shadow. The backlog got the theme-coloured `border-l-2` treatment in `3248b74`, but nested rows can't have one: it would be confused with the CSS-border ancestry rule (option C). Needs a different cue — brighter fill, or a tinted marker.
9. **The whole 2026-08-06 batch is UNRUN.** See the Roadmap note below — this is the single biggest risk carried forward, not any individual item here.

## Roadmap

**⚠ Verification debt is the top risk.** Eight commits landed on 2026-08-06 (`1a9a332` → `e90704c`). Of that batch only two things were exercised in a running app: the backlog-selection highlight, and `/deadline` + `/deadline off` — both via `npm run dev`, both reported fine. **Everything else is tsc + `vite build` + read-through only**, including the two mechanical CSS verifications described under Wave 2. No `tauri build` has been made since the identifier change, so the installed app is still older than all of it. Assume nothing in this batch has been felt.
...
**Wave 1 — DONE 2026-08-06** (except the `/export md` noted in 3).

0. **Not on the original list, found while doing it: `useDeadlineColor` emitted `hsl(NaN, 70%, NaN%)` whenever no deadline was set** — which is the default. `''.split(':').map(Number)` is `[0]`, so `dm` is `undefined`, `deadlineMinutes` is `NaN`, both comparisons fall through, and the ramp branch computed `NaN`. Every `var(--theme-color)` then resolved to an invalid value: text fell back to the inherited `:root { color: #00ff00 }`, a harsher neon green close enough to the intended `hsl(142,70%,45%)` that nobody noticed, while the `color-mix()` borders and glows became invalid declarations and were dropped — no panel border, no ball glow. Guarded with `isFinite`, and the resting color is now the shared `THEME_REST` constant. **This was a prerequisite for item 1**: `/deadline off` walks users into that exact state deliberately. It also matters for Wave 2, where the ramp endpoints come from the preset.
1. ~~`/deadline off`~~ — **DONE.** Also matches bare `/deadline` (which previously fell through to task capture and added a task named `/deadline`), accepts `off`/`clear`/`none`, and posts a notice on malformed input instead of swallowing it.
2. ~~Version string hardcoded in About tab~~ — **DONE.** `vite.config.ts` reads `package.json` and injects `__APP_VERSION__` via `define` (build-time, so the manifest stays out of the bundle); declared in `src/vite-env.d.ts`. **`src-tauri/tauri.conf.json` still carries its own copy** — that one keys the installer upgrade path, so keep both in step when releasing.
3. ~~Archive markdown only written at rollover *while running*~~ — **this entry was wrong.** `runDayCheck()` runs on mount as well as on `focus`/`visibilitychange`, so quitting before midnight only *defers* the write: next launch still sees `geek-last-date` as yesterday and archives it from the untouched `geek-tasks`. The only real loss case is never opening the app again. What was actually broken here — the rollover archiving **top-level completed tasks only**, so a day of subtask-only progress archived nothing at all and never even called `exportDailyLog` — is **FIXED 2026-08-06** via `completedLeaves` (flat, for the LOG tab and weekStats, which render and count one line per element) and `completedTree` (pruned, hierarchy intact, for the recursive markdown emitter). Still genuinely open: an `/export md` for the *current* day, since today's log is only obtainable by waiting for rollover.
4. ~~Duplicate-date archive entries~~ — **DONE.** The rollover now *replaces* an entry matching the date instead of appending a second one. The LOG tab and `weekStats` both iterate entries rather than dates, so a duplicate rendered the day twice and double-counted it in the 7-day total. Reachable whenever `geek-last-date` moves backwards — the disaster-recovery restore rewrites it from the disk mirror.

**Wave 2 — themes: steps 1-4 SHIPPED 2026-08-06, step 5 (audit) OUTSTANDING.** `/theme` · `/theme <name>` · `/theme ramp on|off`; presets `default`, `mono`, `ice`. Eight accent variables (`--accent-action{,-soft,-deep,-wash}`, `--accent-edit{,-dim}`, `--accent-danger`, `--accent-affirm`) set as inline style on the root div beside `--theme-color`; every former Tailwind accent class now reads `[var(--accent-*)]`. `default` is verified byte-identical to the pre-theme app by diffing built CSS with the var names substituted back. **New hard requirement: WebView2 ≥ 111** — Tailwind cannot pre-bake alpha into a hex fallback when the color is a `var()`, so transparency now rides entirely on the `@supports (color-mix)` block (spec §8.3). See **`PLAN_wave2_themes.md`** — that file is binding and supersedes this paragraph. Decided: **preset palette** (each preset ships its own accent set), **per-preset deadline ramp** with an off switch, driven by `/theme` commands because invariant #1 forbids adding focusable elements. The load-bearing finding is that `green-400` (≈142) and `red-400` (≈0) *are* the two endpoints `useDeadlineColor` ramps between, so the accents already collide with the theme at both ends of the ramp — which is what rules out a free hue picker and forces the four accent roles to become CSS variables before any preset can ship. A **full** custom preset (user supplies all five colors) is acceptable and is a later fold-in; a single hue knob is not.
...
- Code: `src/App.tsx` (everything, ~2990 lines), `src/App.css` (panelIn 220ms, panelOut 150ms — matches the collapse timer, ballIn, modalIn, soft-breathe), `src-tauri/src/lib.rs` (tray+plugins), `src-tauri/capabilities/default.json`, `src-tauri/tauri.conf.json`, `src-tauri/Cargo.toml`.
